    ...
    public void AddLabels()
    {
        ...
        foreach (InGameTextEditor.Line line in textEditor.Lines)
        {
            ...
            while (true)
            {
                ...
                int startIndex = line.Text.IndexOf("PLACE", endIndex);
                if (startIndex >= 0)
                {
                    // add label with underline and icon
                    endIndex = startIndex + "PLACE".Length;
                    // AddLabel(startIndex, startIndex, ...), will remove the line
                    // Empty string will remove the tooltip
                    line.AddLabel(startIndex, endIndex, underlineSprite, new Color(0f, 0.8f, 0f, 0.8f), labelIconSprite, new Color(0f, 0.8f, 0f, 1f), "Place an item in an appliance.", InGameTextEditor.Line.Label.DeleteCondition.THIS_LINE_CHANGES);
                }
                else
                    break;
            }
